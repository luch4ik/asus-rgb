...
class KeyboardLightingWindow(QMainWindow):
    ...
    def kill_old_instance(self):
        """Kill any old instance of the application"""
        if PID_FILE.exists():
            try:
                ...
                try:
                    ...
                    logger.info(f"Found old instance with PID {old_pid}, killing it...")
                    os.kill(old_pid, signal.SIGTERM)
                    time.sleep(0.2)  # Give it time to exit gracefully
                    # Check if still alive
                    try:
                        os.kill(old_pid, 0)
                        # Still alive, force kill
                        logger.warning(f"Old instance didn't exit gracefully, forcing...")
                        os.kill(old_pid, signal.SIGKILL)
                    except ProcessLookupError:
                        logger.info(f"Old instance (PID {old_pid}) terminated successfully")
                except ProcessLookupError:
    ...
    def closeEvent(self, event):
        ...
        try:
            self.persist_state()
            # Clean up PID file only if it's ours
            if PID_FILE.exists():
                try:
                    current_pid = int(PID_FILE.read_text().strip())
                    if current_pid == os.getpid():
                        ...
                        logger.info(f"PID file belongs to another instance ({current_pid}), not removing")
                except (ValueError, FileNotFoundError):
                    pass
            # Don't stop animations - let them continue running
        finally:
